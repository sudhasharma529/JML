...
%OPTIONA CODE TO COMPENSATE ERROR INTRODUCED BY THE BRIGHTLED ON THE EYELID
%POSITION
ligth_off_on_frame = round((metadata.cam.time(1) + metadata.stim.c.csdelay) * metadata.cam.fps / 1000.0);
ligth_error = y(ligth_off_on_frame+1) - y(ligth_off_on_frame);
for i = ligth_off_on_frame + 1 : ligth_off_on_frame + 1 + round(metadata.stim.c.csdur * metadata.cam.fps / 1000.0)
    y(i) = y(i)-ligth_error;
end
